<script>
	...
	async function getExpiringENS() {
		try {
			const result = await axios.post('https://api.thegraph.com/subgraphs/name/ensdomains/ens', {
				query: `
			...
			const itemsWithCountdown = result.data.data.registrations.map((item) => {
				return {
					...item,
					countdown: intervalToDuration({ start: item.expiryDate * 1000, end: Date.now() })
				};
			});
				...
				{:then expiringENS}
					{#each items as item, index}
						<tr>
							...
							<td>
								<a class="link" href="https://app.ens.domains/name/{item.domain.name}/details">ENS Domains</a>
								<a class="link" href="https://opensea.io/collection/ens?search[query]={item.domain.name}">Opensea</a>
							</td>
						</tr>
					{/each}
				{/await}
